...
begin
  if FileExists ('mdfe.exe') then Winexec('mdfe.exe',sw_shownormal) else Showmessage('Módulo não Localizado'#13'MDFe');
end;
...
begin
case MessageBox(Handle, 'Deseja realmente Revogar sua licença de Uso?',
    'Revoga Licença', MB_YESNO + MB_ICONQUESTION) of
  ...
  begin
    ...
    showmessage('Atenção'#13'Sistema precisa ser finalizado para que as alterações entrem em vigor');
    close;
    close;
  end;
...
begin
MessageDlg('Uma instância do Alles está em execução.' + #13 +
'Não é possível abrir outra instância.', mtInformation, [mbOk],0);
Halt;
end;}
...
begin
  ...
  begin
    ...
    begin
      mostratelaaviso('NÃO LOCALIZADO A LICENÇA','Não foi possível localizar a sua licença de uso no seu computador');
    end;
  ...
  begin
    ...
    if dm_dados.temnet<>0 then
    begin
      mostratelaaviso('SEM ACCESSO A INTERNET','Não conseguimos acessar nossos servidores para verificar sua licença');
      frm_login.Close;
      frm_principal.Close;
    end
    // COM INTERNET
  ...
  begin
    ...
    begin
      ...
      if (caos>=29) then
      begin
        mostratelaaviso('LIMITE DE ACESSO','Seu sistema atingiu o número limite de conexão com o nosso Servidor. Verifique sua conexão a internet ');
        frm_login.Close;
        frm_principal.Close;
        close;
      end
    ...
    begin
      ...
      begin
        mostratelaaviso('LICENCA NAO LOCALIZADA','Nao foi localizado esse número de licença em nosso servidor.'+#13+'Solicite Suporte Técnico para a licenca número '+licenca);
        frm_login.Close;
        frm_principal.Close;
        close;
      end
      ...
      begin
        ...
        begin
          mostratelaaviso('LICENCA ESTÁ INATIVA','Solicitamos que entre em contato com a Revenda ou com a NonoElemento.'+#13+'Solicite Suporte Técnico para a licenca número '+licenca);
          frm_login.Close;
          frm_principal.Close;
          close;
        end;
        ...
        begin
          mostratelaaviso('LICENCA FOI REVOGADA','Solicitamos que entre em contato com a Revenda ou com a NonoElemento.'+#13+'Solicite Suporte Técnico para a licenca número '+licenca);
          deletefile('configura.exe');
          deletefile('sync.exe');
          frm_login.Close;
          frm_principal.Close;
    ...
    begin
      showmessage('Atenção!!!'#13'Já existe um acesso ativo para o terminal '+inttostr(dm_dados.terminal));
      close;
      close;
    end;
    frm_login.showmodal;
