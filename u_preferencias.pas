...
begin
  ...
  begin
    showmessage('Parametro não encontrado');
  end
  else
  begin
    ...
    // Verifica se o arquivo LOGALLES.TXT existe, se não existir, cria-o
    if not FileExists(nomearquivo) then
    begin
      assignfile(arquivo, nomearquivo);
      rewrite(arquivo); // Cria o arquivo se não existir
      closefile(arquivo);
    end;
    ...
    // Escreve no arquivo
    writeln(arquivo, 'Em ' + datetostr(date) + ' às ' + timetostr(time) + #13);
    writeln(arquivo, 'Alteração na Sequência da NFCe do N.º ' + inttostr(retseq));
    // Fecha o arquivo
    closefile(arquivo);
  end;
end;
...
begin
  ...
  assignfile(arquivo, nomearquivo);
  append(arquivo);
  writeln(arquivo, 'Para o Número N.º '+dbedit2.Text+#13);
  closefile(arquivo);
  zqry_seqnfce.open;
  zqry_SEQNFCE.Post;
...
begin
  ...
  showmessage('Atualização da Sequência de NFe salva');
