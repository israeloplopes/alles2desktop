...
procedure ExibirErroClassificacao;
begin
  if (MessageDlg('O Produto tem um erro na classificação fiscal. Para mais informações clique em OK',
...
begin
  ...
  BEGIN
    ...
    if dm_dados.zqry_fnreceber.IsEmpty then Showmessage('Não existem valores do Contas a Receber que coincidem com a busca');
  END;
  ...
  BEGIN
    ...
    if dm_dados.zqry_FNPAGAR.IsEmpty then Showmessage('Não existem valores do Contas a Pagar que coincidem com a busca');
  END;
  ...
  begin
    ...
    begin
      ...
      if dm_dados.zqry_CPCOMPRAcodtipomov.asinteger=212 then Ide.natOp     := 'DEVOLUÇÃO DE COMPRA';
      ...
      if dm_dados.zqry_VDVENDAcodtipomov.asinteger=560  then Ide.natOp     := 'INDUSTRIALIZAÇÃO PARA DESTINATÁRIO';
      if dm_dados.zqry_VDVENDAcodtipomov.asinteger=570  then Ide.natOp     := 'CONSIGNAÇÃO';
      if dm_dados.zqry_VDVENDAcodtipomov.asinteger=572  then Ide.natOp     := 'BONIFICAÇÃO';
      ...
      if dm_dados.zqry_VDVENDAcodtipomov.asinteger=585  then Ide.natOp     := 'AMOSTRA GRÁTIS';
   ...
   // if dm_dados.ambiente='0' then frmacbrnfe.acbrnfe
    ...
    if dm_dados.ambiente='1' then Ide.tpAmb     := taHomologacao;  //Lembre-se de trocar esta variável quando for para ambiente de produção
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            showmessage('Código do Produto com erro na Classificação é '+frm_CON_PEDIDOS.zqry_VDITVENDACODPROD.asstring+' e o NCM dele é '+frm_con_pedidos.zqry_VDITVENDACODFISC.asstring);
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  //showmessage('Atenção'#13'Regra Fiscal alterada para este pedido');
                end;
                if frm_CON_PEDIDOS.TIPOMOVIMENTO=1 then
                begin
...
*)
  ...
  *)
  ...
  begin
    ...
    if dm_dados.zqry_FNRECEBER.IsEmpty  then showmessage('Contas a Receber não foram localizadas');
    ...
    if dm_dados.zqry_FNITRECEBER.IsEmpty then showmessage('Itens do Contas a Receber não foram localizados');
...
// Exemplo de pagamento integrado.
  ...
  infRespTec.xContato := 'Israel de Oliveira Pinheiro Lopes'; // Nome do responsável técnico
...
begin
  ...
  begin
    ...
    MostraTelaAguarde('Enviando DANFE e XML no email Repositório');
  end;
...
//      if dm_dados.zqry_SGFILIALEMAILRESP.AsString<>'' then cc.Add(dm_Dados.zqry_SGFILIALEMAILRESP.asstring);
    ...
    frmacbrnfe.ACBrMail1.UseThread := true;           //Aguarda Envio do Email(nÃ£o usa thread)
...
begin
  ...
  begin
    ...
    if frmacbrnfe.ACBrNFe1.WebServices.Retorno.cStat<>100 then
    begin
      mostratelaaviso('NOTA NÃO EMITIDA','Consulte o motivo do CSTAT '+inttostr(frmacbrnfe.ACBrNFe1.WebServices.Retorno.cStat)+' no alto da tela');
    end;
  end;
  {else
...
begin
  ...
  begin
    ...
    frm_CON_PEDIDOS.ufigual:=0;   // ufigual 0 é mesmo estado
  end
  else
  begin
    frm_CON_PEDIDOS.larq.add('iddest=2');
    frm_CON_PEDIDOS.ufigual:=1;  // uf igual 1 é outro estado
  end;
 ...
 { if dm_Dados.teminternet<>0 then
  begin
    ...
    frm_CON_PEDIDOS.larq.add('xJust=Nota Emitida em Contingencia. Problema Técnico');
  end;}
  ...
  begin
    ...
    if dm_dados.zqry_VDVENDAcodtipomov.asinteger=572 then frm_CON_PEDIDOS.larq.add('NaturezaOperacao=BONIFICAÇÃO');
...
begin
  ...
  begin
    ...
    //CONSULTA PRODUTO PARA SABER SE É MIX
...
begin
  ...
  begin
    ...
    frm_CON_PEDIDOS.zqry_itfiscal.open;
    if (frm_CON_PEDIDOS.zqry_itfiscal.IsEmpty) and (frm_CON_PEDIDOS.TIPOMOVIMENTO=0) then showmessage('Atenção'#13'Não existe Descritivo de Item na Classificação Fiscal cadastrada para esse NCM de acordo com o movimento escolhido'+ trim(DM_DADOS.zqry_VDITVENDA_NCMPRODUTO.VALUE));
    if (frm_CON_PEDIDOS.zqry_itfiscal.IsEmpty) and (frm_CON_PEDIDOS.TIPOMOVIMENTO=1) then showmessage('Atenção'#13'Não existe Descritivo de Item na Classificação Fiscal cadastrada para esse NCM de acordo com o movimento escolhido'+ trim(DM_DADOS.zqry_CPITCOMPRA_NCMPRODUTO.VALUE));
...
begin
  ...
  begin
    ...
    // 5949 e 6949 = Simples Remessa | 5910 e 6910 = Bonificação
...
//////////////////////////////////////////////////
  ...
  begin
     if dm_dados.zqry_sgfilialsimplesfilial.asstring='S' then frm_CON_PEDIDOS.mensagemnfe:='DOCUMENTO EMITIDO POR ME OU EPP OPTANTE PELO SIMPLES NACIONAL E NÃO GERA DIREITO A CRÉDITO FISCAL DE ICMS, DE ISS E DE IPI |';
     if dm_dados.zqry_sgfilialsimplesfilial.asstring='N' then frm_CON_PEDIDOS.mensagemnfe:='DOCUMENTO EMITIDO COM ICMS POR SUBSTITUIÇÃO TRIBUTARIA -COM DIREITO A CRÉDITO DE ICMS |';
  end
  else
  begin
  ...
  if (frm_CON_PEDIDOS.TIPOMOVIMENTO=1) then frm_CON_PEDIDOS.larq.add('Texto= Não Informado');
  if (frm_CON_PEDIDOS.TIPOMOVIMENTO=0) then
  begin
