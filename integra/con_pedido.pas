...
begin
  if (MessageDlg('Nota DENEGADA ou em PROCESSAMENTO. Anote a Chave para consulta futura. Para mais informações clique em OK',
...
begin
  ...
  try
    frmacbrnfe.lerconfiguracao;
    cc.Add('fiscal.nonoelemento@gmail.com'); //especifique um email vÃ¡lido
...
//      if dm_dados.zqry_SGFILIALEMAILRESP.AsString<>'' then cc.Add(dm_Dados.zqry_SGFILIALEMAILRESP.asstring);
    ...
    frmacbrnfe.ACBrMail1.SetSSL := frmacbrnfe.cbEmailSSL.Checked; // SSL - ConexÃ£o Segura
    frmacbrnfe.ACBrMail1.SetTLS := frmacbrnfe.cbEmailTLS.Checked; // Auto TLS
    frmacbrnfe.ACBrMail1.ReadingConfirmation := False; //Pede confirmaÃ§Ã£o de leitura do email
    frmacbrnfe.ACBrMail1.UseThread := False;           //Aguarda Envio do Email(nÃ£o usa thread)
...
begin
  ...
  begin
    ...
    begin
      mostratelaaviso('CLIENTE NÃO LOCALIZADO','Código Cliente '+zqry_vdvendacodcli.asstring);
    end;
    ...
    begin
      ...
      begin
        mostratelaaviso('PEDIDO NÃO PODE SER EMITIDO COMO NFE','');
        FECHACONEXOES;
      end
      else
      begin
  ...
  begin
    mostratelaaviso('PEDIDO JÁ FATURADO','Chave '+zqry_VDVENDACHAVENFEVENDA.asstring);
  end;
end;
...
ATUALIZACLIENTE;
if zqry_VDCLIENTE.IsEmpty then showmessage('Dados não localizados');
...
with frmacbrnfe.ACBrNFe1.NotasFiscais.Add.NFe do
  begin
    Ide.cNF       := random(99999999)+1;//StrToInt(NumNFe); //Caso não seja preenchido será gerado um número aleatório pelo componente
    Ide.natOp     := 'VENDA';
    Ide.indPag    := ipVista;
    Ide.modelo    := 65;
    Ide.serie     := 1;
    ...
    Ide.tpAmb     := TpcnTipoAmbiente(frmacbrnfe.rgTipoAmb.ItemIndex);  //Lembre-se de trocar esta variÃ¡vel quando for para ambiente de produÃ§Ã£o
...
//Adicionando Produtos
    ...
    begin
      ...
      begin
         ...
         Prod.nItem    := contador;
         //showmessage(inttostr(contador));// Número sequencial, para cada item deve ser incrementado
       ...
       { codigoproduto:=zqry_VDITVENDACODPROD.AsInteger;
        ...
        Prod.NCM      := NCM;//zqry_VDITVENDA_NCMPRODUTO.asstring; // Tabela NCM disponível em  http://www.receita.fazenda.gov.br/Aliquotas/DownloadArqTIPI.htm
        Prod.EXTIPI   := '';
        Prod.CFOP     := zqry_VDITVENDACODNAT.ASSTRING;
...
begin
   MOSTRATELANOTIFICA('PEDIDO NÃO LOCALIZADO','Escolha um pedido');
end
else
begin
  zqry_contaqtd.Close;
...
BEGIN
 mostratelaaviso('ESTE PEDIDO JÁ FOI FATURADO','');
END;
...
BEGIN
 ...
 { if not(InputQuery('WebServices Enviar', 'Envio Síncrono(1=Sim, 0=Não)', vSincrono)) then
  ...
  //MOSTRATELANOTIFICA('NFCE '+vaux+' será impressa na Impressora Padrão','Atenção para impressão...');
  ...
  begin
    ...
    lbl_SITUACAO.caption:='Situação '+inttostr(frmacbrnfe.ACBrNFe1.WebServices.Retorno.cStat);
    zqry_MAX.Close;
    zqry_MAX.SQL.Clear;
    ...
    MostraTelaAguarde('Próxima Nota a ser emitida n.º '+ inttostr(numnf+1));
    dm_Dados.zqry_SGSEQUENCIA.Close;
    fechatelaaguarde;
  ...
  begin
    SHOWMESSAGE('NFCE não emitida'#13'Verifique sua conexão a internet ou a conexão com a Sefaz'#13'CLIQUE NOVAMENTE NO BOTÃO DE EMISSÃO');
    ZerarTxt('..\nfe\log\log_nfe.txt');
    ZerarTxt('..\nfe\log\log_comp.txt');
    Zerartxt('..\nfe\log_nfe.txt');
    fechaconexoes;
...
begin
  MOSTRATELAAVISO('ESTE PEDIDO NÃO É EDITÁVEL','');
end;
END;
end;
...
if dm_dados.zqry_CPCOMPRACODTIPOMOV.AsInteger>=601 then mostratelaaviso('ESTE PEDIDO JÁ FOI FATURADO','');
...

{
  ...
  if dm_dados.zqry_VDVENDACODTIPOMOV.AsInteger>=601 then mostratelaaviso('ESTE PEDIDO JÁ FOI FATURADO','');
...
var
  erro,path:string;
begin
// AJUSTA PROPRIEDADES DE ENVIO DA NFE NAS OPÇÕES DA INTERNET
...
//    lerlog  :=TIniFile.Create('C:\Sistema\Integra48\Win32\nfe\sai\entnfe-resp.txt');
  ...
  //  showmessage('parte 1 - Gravar na SGNF');
      ...
      dm_dados.zqry_SGNF.post;
      ExibirDenegada;
      //showmessage('Atenção Usuario'#13'Nota DENEGADA ou em PROCESSAMENTO'#13'Anote a Chave para consulta futura');//
      ...
      dm_dados.zqry_sgsequencia.execsql;
      MostraTelaAguarde('Nota não emitida. *|DENEGADA|*. Próxima Nota a ser emitida n.º '+ inttostr(numnf+1));
      dm_Dados.zqry_SGSEQUENCIA.Close;
  ...
  //  showmessage('parte 1 - Gravar na SGNF');
      ...
      begin
        mostratelaaguarde('Guardando XML conforme legislação');
        idftp1.Disconnect();
    ...
    begin
      showmessage('Nota Fiscal não emitida');
      FECHACONEXOES;
    end;
end;
  ...
  while i_Tam_Arq = 0 do
   ...
   // showmessage(lbl_cstat.caption+' variavel '+nfecstat);
    ...
    begin
      ...
      dm_Dados.zqry_sgsequencia.ParamByName('empcod').AsInteger:=dm_DADOS.CODEMP;
      dm_dados.zqry_sgsequencia.execsql;
      showmessage('Nota não emitida. *|DENEGADA|*. Próxima Nota a ser emitida n.º '+ inttostr(numnf+1));
      dm_Dados.zqry_SGSEQUENCIA.Close;
      dm_Dados.zqry_SGSEQUENCIA.Open;
    //  fechatelaaguarde;
    end;
    ...
    begin
      ...
      fechatelaaguarde;
      // GRAVA VALOR NAS PARCELA RECICMPF=1 E NÃO É DEVOLUÇÃO
      try
      ...
      // GRAVA VALOR NAS PARCELAS RECICMPF<> 1 E NÃO É DEVOLUCAO
      try
        if (recicmpf<>1) and (zqry_VDVENDACODTIPOMOV.asinteger<>202) then
        begin
          MostraTelaAguarde('Gravandos dados Valores das Parcelas no Contas a Receber');
...
begin
  ...
  begin
    log := emptystr;
    if not(InputQuery('Alles: Motivo de Reemissão', 'Motivo da Reemissão', log)) then
     exit;
    tipodoc:=copy(zqry_VDVENDACHAVENFEVENDA.asstring,21,2);
...
redicms:=0;
recicmpf:=0;
MostraTelaAguarde('Consultando Informações do Pedido '+zqry_VDVENDACODVENDA.asstring);
//
...
//
fechatelaaguarde;
MostraTelaAguarde('Consultando Informações do Pedido '+zqry_VDVENDACODVENDA.asstring+' detalhamento Financeiro');
dm_dados.zqry_FNRECEBER.Close;
...
fechatelaaguarde;
  ...
  begin
    mostratelaaviso('PEDIDO LOCALIZADO MAS SEM FECHAMENTO','Não localizado Venda x Entrega');
  end
  else
  begin
    ...
    begin
      ...
      MostraTelaAviso('Necessário Informar Chave de Referência para Devolução','');
    end;
  end;
///
...
recicmpf:=0;
//
MostraTelaAguarde('Consultando Informações do Pedido '+dm_dados.zqry_CPCOMPRACODCOMPRA.asstring);
dm_Dados.zqry_SGFILIAL.Close;
dm_Dados.zqry_SGFILIAL.SQL.Clear;
dm_Dados.zqry_SGFILIAL.SQL.Add('select * from sgfilial where codemp=:empcod');
dm_dados.zqry_SGFILIAL.ParamByName('empcod').AsInteger:=dm_DADOS.CODEMP;
...
fechatelaaguarde;
MostraTelaAguarde('Consultando Informações do Pedido '+dm_dados.zqry_CPCOMPRACODCOMPRA.asstring+' detalhamento Financeiro');
...
begin
  showmessage('Quantidade de Entrega não permitida para esta venda');
  edt_QTDENTREGA.Text:='0';
...
zqry_VDVENDA.Open;
if zqry_VDVENDA.IsEmpty then mostratelaaviso('Nada localizado com base nas informações','tab.: vdvenda');
zqry_VDCLIENTE.Close;
zqry_VDCLIENTE.sql.clear;
zqry_VDCLIENTE.SQL.Add('select codemp, codfilial, codcli, razcli, nomecli, pessoacli,');
...
begin
  ...
  if not(InputQuery('Alles: Carta de Correção', 'Correção a ser considerada', Correcao)) then
   exit;

// Verificar se o texto tem pelo menos 15 caracteres
  if Length(Correcao) < 15 then
   begin
    ShowMessage('O texto deve ter pelo menos 15 caracteres. Por favor, forneça uma correção válida.');
   exit;
  end;
  frmacbrnfe.ACBrNFe1.EventoNFe.Evento.Clear;
  ...
  begin
    ...
    zqry_UPDDEL.ExecSQL;
    mostratelaaviso('Carta de Correção Gerada com sucesso ',retmotivo);
  ...
  begin
    mostratelaaviso('Carta de Correção Não Gerada ',retmotivo);
  end;
end;
...
{    if Trim(vNumLote) = '' then
    ...
    if (Trim(vSincrono) = '1') then
      ...
      begin
       ...
       begin
        SHOWMESSAGE('CSTAT de NAO APROVAÇÃO');
      end;
    end
    else
    begin
...
begin
 mostratelaaviso('Nada localizado com base nas informações','tab.: cpcompra');
end;
dm_dados.zqry_CPFORNECED.Close;
dm_Dados.zqry_cpfornecED.sql.clear;
...
dm_dados.zqry_cpcompra.ParamByName('empcod').asinteger:=dm_DADOS.CODEMP;
dm_dados.zqry_cpcompra.Open;
if dm_dados.zqry_cpcompra.IsEmpty then mostratelaaviso('Nada localizado com base nas informações','tab.: cpcompra');
dm_dados.zqry_CPFORNECed.Close;
dm_Dados.zqry_CPFORNECed.sql.clear;
dm_Dados.zqry_CPFORNECed.SQL.Add('select * from CPFORNECED where codemp=:empcod');
dm_Dados.zqry_CPFORNECed.ParamByName('empcod').AsInteger:=dm_DADOS.CODEMP;
...
begin
 case rg_ENTRADA.ItemIndex of
    0:begin
      LBL_PESQUISA_ENT.Caption:='ORDENANDO POR CÓDIGO';
      msk_PESQUISA1_ENT.Visible:=False;
      msk_PESQUISA2_ENT.VISIBLE:=FALSE;
      LBL_ATE_ENT.VISIBLE:=FALSE;
      ENTRE:='N';
    ...
    3:begin
      FECHACONEXOES;
      LBL_PESQUISA.Caption:='BUSCA PEDIDOS PARA DEVOLUÇÃO';
      msk_PESQUISA1_ent.Visible:=False;
      msk_PESQUISA2_ent.VISIBLE:=FALSE;
      LBL_ATE_ent.VISIBLE:=FALSE;
      ENTRE:='N';
...
begin
 ...
 case rg_FUTURA.ItemIndex of
    0:begin
      FECHACONEXOES;
      LBL_PESQUISAFUTURA.Caption:='ORDENANDO POR CÓDIGO';
      msk_PESQUISAFUTURA1.Visible:=False;
      msk_PESQUISAFUTURA2.VISIBLE:=FALSE;
      LBL_ATEFUTURA.VISIBLE:=FALSE;
      ENTRE:='N';
