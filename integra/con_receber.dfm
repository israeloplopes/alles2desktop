object frm_CON_RECEBER: Tfrm_CON_RECEBER
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'INTEGRA - CONTAS A RECEBER'
  ClientHeight = 726
  ClientWidth = 850
  Color = clWhite
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  ShowHint = True
  OnActivate = FormActivate
  OnClose = FormClose
  OnCreate = FormCreate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object btn_BLOG: TBitBtn
    Left = 712
    Top = 528
    Width = 75
    Height = 25
    Caption = 'btn_BLOG'
    TabOrder = 0
    Visible = False
    OnClick = btn_BLOGClick
  end
  object pg_BOLREC: TPageControl
    Left = 3
    Top = 3
    Width = 840
    Height = 720
    ActivePage = ts_BOLRECIBO
    TabOrder = 2
    object ts_BOLRECIBO: TTabSheet
      Caption = 'Boleto/Recibo/Promiss'#243'ria'
      OnEnter = ts_BOLRECIBOEnter
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object lbl_MENS1: TLabel
        Left = 3
        Top = 254
        Width = 572
        Height = 14
        Caption = 
          'Use um duplo-clique na caixa acima para selecionar o T'#237'tulo. Sel' +
          'ecione a Primeira Parcela na caixa abaixo'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object lbl_MENS2: TLabel
        Left = 3
        Top = 265
        Width = 482
        Height = 14
        Caption = 
          'Para apenas um parcela, selecione a parcela desejada e no Passo ' +
          '2'#186'  confirme com N'#195'O'
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object rg_CONRECEBER: TRadioGroup
        Left = 3
        Top = 0
        Width = 697
        Height = 47
        Caption = 'Selecione a Busca'
        Columns = 4
        Items.Strings = (
          'Por Data de Emiss'#227'o'
          'Por Nome/Razao'
          'Por C'#243'd. Cliente'
          'Por C'#243'd. Recebimento'
          'Por C'#243'd. Pedido')
        TabOrder = 0
        OnClick = rg_CONRECEBERClick
      end
      object gp_BOL_PARAMETROS: TGroupBox
        Left = 3
        Top = 50
        Width = 697
        Height = 47
        Caption = 'Par'#226'metros'
        TabOrder = 1
        object lbl_PESQUISA: TLabel
          Left = 8
          Top = 22
          Width = 94
          Height = 13
          Caption = 'Data Emiss'#227'o Inicial'
        end
        object lbl_ATE: TLabel
          Left = 270
          Top = 22
          Width = 19
          Height = 13
          Caption = 'AT'#201
        end
        object Label5: TLabel
          Left = 8
          Top = 22
          Width = 138
          Height = 13
          Caption = 'Selecione o Nome do Cliente '
        end
        object sp_BUSCA: TSpeedButton
          Left = 470
          Top = 20
          Width = 23
          Height = 22
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          OnClick = sp_BUSCAClick
        end
        object msk_PESQUISA1: TMaskEdit
          Left = 170
          Top = 20
          Width = 63
          Height = 21
          TabOrder = 0
          Text = ''
        end
        object msk_PESQUISA2: TMaskEdit
          Left = 300
          Top = 20
          Width = 65
          Height = 21
          TabOrder = 1
          Text = ''
        end
        object DBLookupComboBox1: TDBLookupComboBox
          Left = 170
          Top = 20
          Width = 279
          Height = 21
          KeyField = 'RAZCLI'
          ListField = 'RAZCLI'
          ListSource = dm_DADOS.zdts_VDCLIENTE
          TabOrder = 2
        end
        object btn_BUSCANOME: TBitBtn
          Left = 480
          Top = 20
          Width = 24
          Height = 24
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          TabOrder = 3
          OnClick = btn_BUSCANOMEClick
        end
        object btn_BUSCARECEBER: TBitBtn
          Left = 560
          Top = 24
          Width = 75
          Height = 25
          Caption = 'btn_BUSCARECEBER'
          TabOrder = 4
          Visible = False
          OnClick = btn_BUSCARECEBERClick
        end
        object btn_BUSCARECEBER_DATA: TBitBtn
          Left = 536
          Top = 16
          Width = 75
          Height = 25
          Caption = 'btn_BUSCARECEBER_DATA'
          TabOrder = 5
          Visible = False
          OnClick = btn_BUSCARECEBER_DATAClick
        end
      end
      object db_BOL_RECEBER: TDBGrid
        Left = 3
        Top = 103
        Width = 698
        Height = 150
        Hint = 
          'Selecionando o T'#237'tulo|D'#234' um duplo clique no t'#237'tulo para selecion' +
          #225'-lo|2'
        DataSource = dm_DADOS.zdts_FNRECEBER
        ReadOnly = True
        TabOrder = 2
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDblClick = db_BOL_RECEBERDblClick
        OnKeyDown = db_BOL_RECEBERKeyDown
        Columns = <
          item
            Expanded = False
            FieldName = 'CODREC'
            Title.Caption = 'COD. REC.'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CODBANCO'
            Title.Caption = 'BANCO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NUMCONTA'
            Title.Caption = 'N.'#186' CONTA'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CODVENDA'
            Title.Caption = 'PEDIDO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = '_PLANOPAG'
            Title.Caption = 'FORMA PAGTO'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CODCLI'
            Title.Caption = 'C'#211'D.'
            Width = 48
            Visible = True
          end
          item
            Expanded = False
            FieldName = '_RAZCLIENTE'
            Title.Caption = 'NOME CLIENTE'
            Width = 200
            Visible = True
          end
          item
            Expanded = False
            FieldName = '_TIPOCOBRANCA'
            Title.Caption = 'COBRAN'#199'A'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DOCREC'
            Title.Caption = 'DOC. REC.'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'OBSREC'
            Title.Caption = 'OBS. REC'
            Width = 300
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRREC'
            Title.Caption = 'VLR. RECEBER'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CODCARTCOB'
            Title.Caption = 'CARTEIRA'
            Visible = True
          end>
      end
      object db_BOL_ITRECEBER: TDBGrid
        Left = 3
        Top = 280
        Width = 697
        Height = 110
        Hint = 
          'Selecionando Parcela|Caso deseja emitir um boleto, verifique os ' +
          'campos de datas, multa e juros abaixo e clique na op'#231#227'o 1'#186' Passo' +
          '|1'
        DataSource = dm_DADOS.zdts_FNITRECEBER
        ReadOnly = True
        TabOrder = 3
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnCellClick = db_BOL_ITRECEBERCellClick
        OnKeyDown = db_BOL_ITRECEBERKeyDown
        Columns = <
          item
            Expanded = False
            FieldName = 'CODREC'
            Title.Caption = 'C'#211'D'
            Width = 40
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NPARCITREC'
            Title.Caption = 'PARC'
            Width = 40
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DESCPONT'
            Title.Caption = 'DESC. PONT'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRJUROSITREC'
            Title.Caption = 'VLR JUROS'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRMULTAITREC'
            Title.Caption = 'VLR MULTA'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRPARCITREC'
            Title.Caption = 'VLR. PARCELA'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRAPAGITREC'
            Title.Caption = 'VLR A PAGAR'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRPAGOITREC'
            Title.Caption = 'VLR PAGO'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DTVENCITREC'
            Title.Caption = 'VENCIMENTO'
            Visible = True
          end>
      end
      object gp_BOLETO: TGroupBox
        Left = 3
        Top = 392
        Width = 697
        Height = 245
        Caption = 'Boleto'
        TabOrder = 4
        DesignSize = (
          697
          245)
        object Label18: TLabel
          Left = 470
          Top = 95
          Width = 55
          Height = 13
          Caption = 'Instru'#231#227'o 2'
        end
        object Label17: TLabel
          Left = 231
          Top = 95
          Width = 55
          Height = 13
          Caption = 'Instru'#231#227'o 1'
        end
        object Label16: TLabel
          Left = 5
          Top = 95
          Width = 51
          Height = 13
          Caption = 'Mensagem'
        end
        object Label15: TLabel
          Left = 460
          Top = 55
          Width = 67
          Height = 13
          Caption = 'Aguardar dias'
        end
        object Label14: TLabel
          Left = 367
          Top = 55
          Width = 80
          Height = 13
          Caption = 'Percentual Multa'
        end
        object Label13: TLabel
          Left = 280
          Top = 55
          Width = 67
          Height = 13
          Caption = 'Data Protesto'
        end
        object Label12: TLabel
          Left = 187
          Top = 55
          Width = 81
          Height = 13
          Caption = 'Data Abatimento'
        end
        object Label11: TLabel
          Left = 96
          Top = 55
          Width = 71
          Height = 13
          Caption = 'Data Desconto'
        end
        object Label10: TLabel
          Left = 5
          Top = 55
          Width = 79
          Height = 13
          Caption = 'Data Mora Juros'
        end
        object Label9: TLabel
          Left = 549
          Top = 15
          Width = 72
          Height = 13
          Caption = 'Valor Desconto'
        end
        object Label8: TLabel
          Left = 458
          Top = 15
          Width = 80
          Height = 13
          Caption = 'Valor Mora Juros'
        end
        object Label7: TLabel
          Left = 5
          Top = 15
          Width = 81
          Height = 13
          Caption = 'Local Pagamento'
        end
        object Label6: TLabel
          Left = 367
          Top = 15
          Width = 82
          Height = 13
          Caption = 'Valor Abatimento'
        end
        object Label4: TLabel
          Left = 551
          Top = 55
          Width = 35
          Height = 13
          Caption = 'Aceite?'
        end
        object Image1: TImage
          Left = 285
          Top = 193
          Width = 166
          Height = 43
          Stretch = True
        end
        object Label1: TLabel
          Left = 5
          Top = 132
          Width = 52
          Height = 13
          Caption = 'Ocorr'#234'ncia'
        end
        object Label2: TLabel
          Left = 5
          Top = 168
          Width = 33
          Height = 13
          Caption = 'Layout'
        end
        object Label20: TLabel
          Left = 457
          Top = 137
          Width = 48
          Height = 13
          Caption = 'CPF/CNPJ'
        end
        object btn_LIMPAR: TBitBtn
          Left = 285
          Top = 164
          Width = 80
          Height = 27
          Caption = 'Limpar'
          Enabled = False
          TabOrder = 20
          OnClick = btn_LIMPARClick
        end
        object btn_GERAREMESSA: TBitBtn
          Left = 371
          Top = 164
          Width = 80
          Height = 27
          Caption = 'Remessa'
          Enabled = False
          TabOrder = 22
          OnClick = btn_GERAREMESSAClick
        end
        object btn_GERAPDF: TBitBtn
          Left = 371
          Top = 137
          Width = 80
          Height = 27
          Caption = 'Gerar PDF'
          Enabled = False
          TabOrder = 21
          OnClick = btn_GERAPDFClick
        end
        object btn_GERAHTML: TBitBtn
          Left = 285
          Top = 137
          Width = 80
          Height = 27
          Caption = 'Gerar HTML'
          Enabled = False
          TabOrder = 19
          OnClick = btn_GERAHTMLClick
        end
        object edt_instru2: TEdit
          Left = 457
          Top = 110
          Width = 179
          Height = 21
          TabOrder = 13
        end
        object edt_instru1: TEdit
          Left = 231
          Top = 110
          Width = 220
          Height = 21
          TabOrder = 12
        end
        object edt_mensagem: TEdit
          Left = 5
          Top = 110
          Width = 220
          Height = 21
          TabOrder = 11
        end
        object edt_dias: TEdit
          Left = 460
          Top = 70
          Width = 85
          Height = 21
          TabOrder = 9
          Text = '3'
          OnExit = edt_diasExit
        end
        object edt_percmulta: TEdit
          Left = 369
          Top = 70
          Width = 85
          Height = 21
          TabOrder = 8
          Text = '2,00'
        end
        object edt_dtprotesto: TEdit
          Left = 278
          Top = 70
          Width = 85
          Height = 21
          TabOrder = 7
        end
        object edt_dtabate: TEdit
          Left = 187
          Top = 70
          Width = 85
          Height = 21
          TabOrder = 6
        end
        object edt_dtdesc: TEdit
          Left = 96
          Top = 70
          Width = 85
          Height = 21
          TabOrder = 5
        end
        object edt_dtmora: TEdit
          Left = 5
          Top = 70
          Width = 85
          Height = 21
          TabOrder = 4
        end
        object edt_vlrdesc: TEdit
          Left = 549
          Top = 30
          Width = 87
          Height = 21
          TabOrder = 3
          Text = '0,00'
        end
        object edt_vlrmora: TEdit
          Left = 458
          Top = 30
          Width = 85
          Height = 21
          TabOrder = 2
          Text = '0,00'
        end
        object edt_localpagto: TEdit
          Left = 5
          Top = 30
          Width = 356
          Height = 21
          TabOrder = 0
          Text = 'Pag'#225'vel em qualquer banco at'#233' o valor limite'
        end
        object edt_vlrabate: TEdit
          Left = 367
          Top = 30
          Width = 85
          Height = 21
          TabOrder = 1
          Text = '0,00'
        end
        object cbxaceite: TComboBox
          Left = 551
          Top = 70
          Width = 85
          Height = 21
          TabOrder = 10
          Items.Strings = (
            'SIM'
            'NAO')
        end
        object btn_INCLUIRBOLETO: TBitBtn
          Left = 186
          Top = 137
          Width = 48
          Height = 100
          Caption = '2'#186' PASSO Inclui'
          Enabled = False
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          Layout = blGlyphTop
          TabOrder = 17
          WordWrap = True
          OnClick = btn_INCLUIRBOLETOClick
        end
        object btn_IMPRIMIRBOLETO: TBitBtn
          Left = 235
          Top = 137
          Width = 48
          Height = 100
          Anchors = [akLeft, akTop, akRight, akBottom]
          Caption = '3'#186' PASSO Imprime'
          Enabled = False
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          Layout = blGlyphTop
          TabOrder = 18
          WordWrap = True
          OnClick = btn_IMPRIMIRBOLETOClick
        end
        object btn_PREBOLETO: TBitBtn
          Left = 137
          Top = 137
          Width = 48
          Height = 100
          Caption = '1'#186' PASSO Prepara'
          Enabled = False
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          Layout = blGlyphTop
          TabOrder = 16
          WordWrap = True
          OnClick = btn_PREBOLETOClick
        end
        object GroupBox1: TGroupBox
          Left = 576
          Top = 137
          Width = 59
          Height = 101
          Caption = 'Banco'
          TabOrder = 23
          Visible = False
          object db_txt_NUMCONTA: TDBText
            Left = 32
            Top = 16
            Width = 62
            Height = 17
            DataField = 'NUMCONTA'
          end
          object Label3: TLabel
            Left = 8
            Top = 16
            Width = 10
            Height = 13
            Caption = 'cc'
          end
        end
        object cbOCORRENCIA: TComboBox
          Left = 5
          Top = 145
          Width = 130
          Height = 21
          TabOrder = 14
          Text = 'Registrar'
          Items.Strings = (
            'Registrar'
            'Baixar'
            'Protestar'
            'Sustar')
        end
        object cbxlayout: TComboBox
          Left = 5
          Top = 185
          Width = 130
          Height = 21
          TabOrder = 15
          OnChange = cbxlayoutChange
        end
        object edtCPFCNPJ: TEdit
          Left = 457
          Top = 156
          Width = 179
          Height = 21
          TabOrder = 24
        end
      end
      object gb_INFORMA: TGroupBox
        Left = 3
        Top = 638
        Width = 697
        Height = 49
        Caption = 'Informa'#231#245'es'
        TabOrder = 5
        object lbl_INFORMA: TLabel
          Left = 8
          Top = 20
          Width = 683
          Height = 19
          AutoSize = False
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -16
          Font.Name = 'Tahoma'
          Font.Style = [fsBold]
          ParentFont = False
        end
      end
      object gp_RECIBO: TGroupBox
        Left = 707
        Top = 0
        Width = 110
        Height = 253
        Caption = 'Recibo/Promiss'#243'ria'
        TabOrder = 6
        object btn_RECIBO: TBitBtn
          Left = 8
          Top = 24
          Width = 96
          Height = 70
          Caption = 'Recibo'
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          TabOrder = 0
          OnClick = btn_RECIBOClick
        end
      end
      object GroupBox3: TGroupBox
        Left = 706
        Top = 280
        Width = 110
        Height = 357
        Caption = 'Remessa por Dia'
        TabOrder = 7
        object Label23: TLabel
          Left = 10
          Top = 20
          Width = 23
          Height = 13
          Caption = 'Data'
        end
        object Label24: TLabel
          Left = 10
          Top = 60
          Width = 29
          Height = 13
          Caption = 'Banco'
        end
        object Conta: TLabel
          Left = 10
          Top = 100
          Width = 29
          Height = 13
          Caption = 'Conta'
        end
        object edt_REMDIA_CODBANCO: TEdit
          Left = 10
          Top = 75
          Width = 80
          Height = 21
          TabOrder = 1
        end
        object btn_INCLUIRDIA: TBitBtn
          Left = 3
          Top = 329
          Width = 104
          Height = 25
          Caption = 'Remessa do Dia'
          TabOrder = 3
          OnClick = btn_INCLUIRDIAClick
        end
        object edt_REMDIA_CONTA: TEdit
          Left = 10
          Top = 115
          Width = 80
          Height = 21
          TabOrder = 2
        end
        object edt_REMDIA_DATA: TMaskEdit
          Left = 8
          Top = 35
          Width = 79
          Height = 21
          EditMask = '99/99/9999'
          MaxLength = 10
          TabOrder = 0
          Text = '  /  /    '
        end
      end
      object gb_DEVEDOR: TGroupBox
        Left = 706
        Top = 638
        Width = 110
        Height = 49
        Caption = 'Saldo Devedor'
        TabOrder = 8
        object DBText3: TDBText
          Left = 3
          Top = 24
          Width = 104
          Height = 17
          Alignment = taRightJustify
          DataField = 'SALDODEVEDOR'
          DataSource = zdts_SALDODEVEDOR
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -15
          Font.Name = 'Arial Narrow'
          Font.Style = [fsBold]
          ParentFont = False
        end
      end
    end
    object ts_CONITREC: TTabSheet
      Caption = 'Contas a Receber'
      ImageIndex = 1
      OnEnter = ts_CONITRECEnter
      ExplicitLeft = 0
      ExplicitTop = 0
      ExplicitWidth = 0
      ExplicitHeight = 0
      object Label22: TLabel
        Left = 3
        Top = 340
        Width = 332
        Height = 14
        Caption = 'Use um duplo-clique na caixa acima para selecionar o T'#237'tulo. '
        Font.Charset = ANSI_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'Arial'
        Font.Style = [fsBold]
        ParentFont = False
      end
      object rg_CONITRECEBER: TRadioGroup
        Left = 3
        Top = 0
        Width = 697
        Height = 47
        Caption = 'Selecione a Busca'
        Columns = 4
        Items.Strings = (
          'Por Data de Emiss'#227'o'
          'Por C'#243'd. Recebimento'
          'Por C'#243'd. Documento'
          'Por Nosso N'#250'mero'
          'Por Valor'
          'Por Data de Vencimento')
        TabOrder = 0
        OnClick = rg_CONITRECEBERClick
      end
      object gp_REC_PARAMETROS: TGroupBox
        Left = 3
        Top = 50
        Width = 697
        Height = 47
        Caption = 'Par'#226'metros'
        TabOrder = 1
        object lbl_PESQUISA1: TLabel
          Left = 8
          Top = 22
          Width = 94
          Height = 13
          Caption = 'Data Emiss'#227'o Inicial'
        end
        object lbl_ATE1: TLabel
          Left = 239
          Top = 22
          Width = 55
          Height = 13
          Alignment = taCenter
          AutoSize = False
          Caption = 'at'#233
        end
        object sp_BUSCA1: TSpeedButton
          Left = 470
          Top = 20
          Width = 23
          Height = 22
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          OnClick = sp_BUSCA1Click
        end
        object msk_REC_PESQUISA1: TMaskEdit
          Left = 170
          Top = 20
          Width = 63
          Height = 21
          TabOrder = 0
          Text = ''
        end
        object msk_REC_PESQUISA2: TMaskEdit
          Left = 300
          Top = 20
          Width = 65
          Height = 21
          TabOrder = 1
          Text = ''
          OnKeyPress = msk_REC_PESQUISA2KeyPress
        end
        object btn_BUSCAITRECEBER: TBitBtn
          Left = 544
          Top = 19
          Width = 75
          Height = 25
          Caption = 'btn_BUSCAITRECEBER'
          TabOrder = 2
          Visible = False
          OnClick = btn_BUSCAITRECEBERClick
        end
      end
      object db_REC_ITRECEBER: TDBGrid
        Left = 3
        Top = 103
        Width = 697
        Height = 230
        DataSource = dm_DADOS.zdts_FNRECEBER
        TabOrder = 2
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnDblClick = db_REC_ITRECEBERDblClick
        Columns = <
          item
            Expanded = False
            FieldName = 'CODREC'
            Title.Caption = 'C'#211'D. REC.'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NPARCITREC'
            Title.Caption = 'N.'#186' PARC.'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRITREC'
            Title.Caption = 'VALOR'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRDESCITREC'
            Title.Caption = 'VLR. DESC.'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRMULTAITREC'
            Title.Caption = 'VLR. MULTA'
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DTITREC'
            Title.Caption = 'DT. EMIS.'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DTVENCITREC'
            Title.Caption = 'DT.VENC.'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DTPREVITREC'
            Title.Caption = 'DT. PREV.'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DTPAGOITREC'
            Title.Caption = 'DT. PAGTO.'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NUMCONTA'
            Title.Caption = 'N.'#186' CONTA'
            Width = 60
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NOSSONUMERO'
            Title.Caption = 'NOSSO N.'#186
            Width = 90
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'OBSITREC'
            Title.Caption = 'OBS.'
            Width = 200
            Visible = True
          end>
      end
      object gp_CLIENTE: TGroupBox
        Left = 3
        Top = 355
        Width = 697
        Height = 47
        Caption = 'Cliente'
        TabOrder = 3
        object Label19: TLabel
          Left = 24
          Top = 16
          Width = 91
          Height = 13
          Caption = 'Raz'#227'o Social/Nome'
        end
        object Label21: TLabel
          Left = 24
          Top = 31
          Width = 110
          Height = 13
          Caption = 'Nome Fantasia/Apelido'
        end
        object DBText1: TDBText
          Left = 152
          Top = 16
          Width = 300
          Height = 17
          DataField = '_RAZCLIENTE'
        end
        object DBText2: TDBText
          Left = 152
          Top = 30
          Width = 300
          Height = 17
          DataField = '_NOMECLIENTE'
        end
      end
      object db_VDVENDA: TDBGrid
        Left = 3
        Top = 405
        Width = 696
        Height = 100
        DataSource = dm_DADOS.zdts_FNITRECEBER
        TabOrder = 4
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'CODVENDA'
            Title.Caption = 'VENDA'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DOCVENDA'
            Title.Caption = 'DOC.'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CODCLI'
            Title.Caption = 'C'#211'D. CLI'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRPRODVENDA'
            Title.Caption = 'VLR. VENDA'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRDESCVENDA'
            Title.Caption = 'VLR. DESC.'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VLRLIQVENDA'
            Title.Caption = 'VLR. LIQ.'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = '_CIDCLIENTE'
            Title.Caption = 'CIDADE'
            Width = 120
            Visible = True
          end
          item
            Expanded = False
            FieldName = '_DDDCLIENTE'
            Title.Caption = 'DDD'
            Visible = True
          end
          item
            Expanded = False
            FieldName = '_FONECLIENTE'
            Title.Caption = 'TEL'
            Width = 70
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'IDUSUINS'
            Title.Caption = 'USU. INS'
            Visible = True
          end>
      end
      object GroupBox2: TGroupBox
        Left = 706
        Top = 0
        Width = 102
        Height = 253
        Caption = 'Remessa/Retorno'
        TabOrder = 5
        object btn_APPREMRET: TBitBtn
          Left = 3
          Top = 24
          Width = 96
          Height = 70
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          TabOrder = 0
          OnClick = btn_APPREMRETClick
        end
      end
      object gb_ULTIMOTITULO: TGroupBox
        Left = 3
        Top = 505
        Width = 826
        Height = 184
        Caption = #218'ltimos T'#237'tulos por Cliente'
        TabOrder = 6
        object Label25: TLabel
          Left = 10
          Top = 18
          Width = 53
          Height = 13
          Caption = 'Data Inicial'
        end
        object Label26: TLabel
          Left = 82
          Top = 18
          Width = 48
          Height = 13
          Caption = 'Data Final'
        end
        object Label27: TLabel
          Left = 16
          Top = 107
          Width = 3
          Height = 13
        end
        object Label28: TLabel
          Left = 10
          Top = 55
          Width = 123
          Height = 13
          Caption = 'Datas em branco para consultar todos'
          Font.Charset = ANSI_CHARSET
          Font.Color = clWindowText
          Font.Height = -9
          Font.Name = 'Arial Narrow'
          Font.Style = []
          ParentFont = False
        end
        object dt_INICIO: TMaskEdit
          Left = 10
          Top = 35
          Width = 66
          Height = 21
          TabOrder = 0
          Text = ''
          OnKeyPress = dt_INICIOKeyPress
        end
        object dt_FIM: TMaskEdit
          Left = 82
          Top = 35
          Width = 66
          Height = 21
          TabOrder = 1
          Text = ''
          OnKeyPress = dt_FIMKeyPress
        end
        object btn_IMP_RELULTTIULO: TBitBtn
          Left = 108
          Top = 106
          Width = 73
          Height = 75
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          TabOrder = 2
          OnClick = btn_IMP_RELULTTIULOClick
        end
        object DBGrid1: TDBGrid
          Left = 187
          Top = 17
          Width = 636
          Height = 164
          TabOrder = 3
          TitleFont.Charset = DEFAULT_CHARSET
          TitleFont.Color = clWindowText
          TitleFont.Height = -11
          TitleFont.Name = 'Tahoma'
          TitleFont.Style = []
          Columns = <
            item
              Expanded = False
              FieldName = 'CODREC'
              Title.Caption = 'C'#211'D. REC.'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'MAX'
              Title.Caption = 'DT. VENCIMENTO'
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'RAZCLI'
              Title.Caption = 'RAZ'#195'O SOCIAL'
              Width = 250
              Visible = True
            end
            item
              Expanded = False
              FieldName = 'VLRREC'
              Title.Caption = 'VLR. PARCELA'
              Visible = True
            end>
        end
        object btn_BUSCAULTTITULOS: TBitBtn
          Left = 154
          Top = 32
          Width = 27
          Height = 25
          Hint = 'Fa'#231'a sua busca por data ou por todos os registros'
          Glyph.Data = {
            
            FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
          TabOrder = 4
          OnClick = btn_BUSCAULTTITULOSClick
        end
      end
    end
  end
  object btn_INCLUI: TBitBtn
    Left = 724
    Top = 398
    Width = 91
    Height = 25
    Caption = 'btn_INCLUI'
    TabOrder = 1
    Visible = False
    OnClick = btn_INCLUIClick
  end
  object FileListBox1: TFileListBox
    Left = 629
    Top = 595
    Width = 78
    Height = 65
    ItemHeight = 13
    TabOrder = 3
    Visible = False
  end
  object ACBrBoleto1: TACBrBoleto
    MAIL = ACBrMail1
    Banco.Numero = 1
    Banco.TamanhoMaximoNossoNum = 7
    Banco.TipoCobranca = cobBancoDoBrasilSICOOB
    Banco.NumeroCorrespondente = 756
    Banco.LayoutVersaoArquivo = 0
    Banco.LayoutVersaoLote = 0
    Banco.CasasDecimaisMoraJuros = 2
    Cedente.CodigoCedente = '4266443'
    Cedente.Agencia = '3787'
    Cedente.AgenciaDigito = '7'
    Cedente.ContaDigito = '0'
    Cedente.TipoCarteira = tctRegistrada
    Cedente.CaracTitulo = tcVinculada
    Cedente.TipoInscricao = pJuridica
    Cedente.IdentDistribuicao = tbBancoDistribui
    NomeArqRemessa = 'remessa.txt'
    DirArqRemessa = '..\remessa\'
    NumeroArquivo = 1
    LeCedenteRetorno = True
    ACBrBoletoFC = ACBrBoletoFCFortes1
    Configuracoes.Arquivos.LogRegistro = False
    Configuracoes.WebService.SSLHttpLib = httpOpenSSL
    Configuracoes.WebService.StoreName = 'My'
    Configuracoes.WebService.Ambiente = taHomologacao
    Configuracoes.WebService.Operacao = tpInclui
    Configuracoes.WebService.VersaoDF = '1.2'
    Left = 648
    Top = 424
  end
  object ACBrBoleto2: TACBrBoleto
    Banco.TamanhoMaximoNossoNum = 10
    Banco.TipoCobranca = cobNenhum
    Banco.LayoutVersaoArquivo = 0
    Banco.LayoutVersaoLote = 0
    Banco.CasasDecimaisMoraJuros = 2
    Cedente.TipoInscricao = pJuridica
    Cedente.IdentDistribuicao = tbBancoDistribui
    NumeroArquivo = 0
    Configuracoes.Arquivos.LogRegistro = False
    Configuracoes.WebService.SSLHttpLib = httpOpenSSL
    Configuracoes.WebService.StoreName = 'My'
    Configuracoes.WebService.Ambiente = taHomologacao
    Configuracoes.WebService.Operacao = tpInclui
    Configuracoes.WebService.VersaoDF = '1.2'
    Left = 648
    Top = 512
  end
  object zqry_SGINICONSP: TZQuery
    Connection = dm_DADOS.zCONECTA
    Params = <>
    Left = 696
    Top = 464
  end
  object ACBrMail1: TACBrMail
    Host = '127.0.0.1'
    Port = '25'
    SetSSL = False
    SetTLS = False
    Attempts = 3
    DefaultCharset = UTF_8
    IDECharset = CP1252
    Left = 699
    Top = 419
  end
  object ACBrFeriado1: TACBrFeriado
    ProxyPort = '8080'
    WebService = wsfCalendario
    Left = 695
    Top = 515
  end
  object zqry_SALDODEVEDOR: TZQuery
    Connection = dm_DADOS.zCONECTA
    SQL.Strings = (
      'select sum(vlrapagrec) as saldodevedor from fnreceber')
    Params = <>
    Left = 649
    Top = 563
    object zqry_SALDODEVEDORSALDODEVEDOR: TExtendedField
      FieldName = 'SALDODEVEDOR'
      ReadOnly = True
      currency = True
      Precision = 19
    end
  end
  object zdts_SALDODEVEDOR: TDataSource
    DataSet = zqry_SALDODEVEDOR
    Left = 697
    Top = 563
  end
  object ACBrBoletoFCFortes1: TACBrBoletoFCFortes
    SoftwareHouse = 'IntegraERP'
    DirLogo = '..\icones'
    Left = 650
    Top = 467
  end
end
