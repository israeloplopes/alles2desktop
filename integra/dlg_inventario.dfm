object frm_DLG_INVENTARIO: Tfrm_DLG_INVENTARIO
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu, biMinimize]
  BorderStyle = bsSingle
  Caption = 'Espelho do Movimento'
  ClientHeight = 561
  ClientWidth = 749
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  ShowHint = True
  OnActivate = FormActivate
  PixelsPerInch = 96
  TextHeight = 13
  object Label1: TLabel
    Left = 8
    Top = 11
    Width = 84
    Height = 13
    Caption = 'Entre os Per'#237'odos'
  end
  object Label2: TLabel
    Left = 175
    Top = 12
    Width = 16
    Height = 13
    Caption = 'at'#233
  end
  object lbl_nomestatus: TLabel
    Left = 536
    Top = 240
    Width = 3
    Height = 13
    Visible = False
  end
  object Label3: TLabel
    Left = 8
    Top = 30
    Width = 90
    Height = 13
    Caption = 'Busque por C'#243'digo'
  end
  object MaskEdit1: TMaskEdit
    Left = 100
    Top = 8
    Width = 70
    Height = 21
    EditMask = '99/99/9999'
    MaxLength = 10
    TabOrder = 0
    Text = '  /  /    '
  end
  object MaskEdit2: TMaskEdit
    Left = 195
    Top = 8
    Width = 69
    Height = 21
    EditMask = '99/99/9999'
    MaxLength = 10
    TabOrder = 1
    Text = '  /  /    '
  end
  object GroupBox1: TGroupBox
    Left = 2
    Top = 114
    Width = 740
    Height = 237
    Caption = 'Registrado para'
    TabOrder = 2
    object CheckListBox4: TCheckListBox
      Left = 537
      Top = 40
      Width = 128
      Height = 194
      ItemHeight = 13
      Items.Strings = (
        '1100 - Compra para Industrializa'#231#227'o / Comercializa'#231#227'o'
        '1101 - Compra para industrializa'#231#227'o'
        '1102 - Compra para comercializa'#231#227'o'
        '1117 - Compra para comercializa'#231#227'o originada de encomenda'
        '1126 - Compra para utiliza'#231#227'o na presta'#231#227'o de servi'#231'o'
        
          '1403 - Compra para comercializa'#231#227'o em opera'#231#227'o com mercadoria su' +
          'jeita a ST'
        
          '1411 - Devolu'#231#227'o de venda de mercadoria adquirida ou recebida de' +
          ' terceiros'
        '1551 - Compra de bem para o ativo imobilizado'
        '1556 - Compra de material para uso ou consumo'
        
          '1916 - Retorno de mercadoria ou bem remetido para conserto ou re' +
          'paro'
        
          '1922 - Lan'#231'amento efetuado a t'#237'tulo de simples faturamento decor' +
          'rente da compra'
        '2100 - Outra UF - Compra para Industrializa'#231#227'o / Comercializa'#231#227'o'
        '2101 - Outra UF - Compra para industrializa'#231#227'o'
        '2102 - Outra UF - Compra para comercializa'#231#227'o'
        
          '2117 - Outra UF - Compra para comercializa'#231#227'o originada de encom' +
          'enda'
        '2126 - Outra UF - Compra para utiliza'#231#227'o na presta'#231#227'o de servi'#231'o'
        
          '2403 - Outra UF - Compra para comercializa'#231#227'o em opera'#231#227'o com me' +
          'rcadoria sujeita a ST'
        
          '2411 - Outra UF - Devolu'#231#227'o de venda de mercadoria adquirida ou ' +
          'recebida de terceiros'
        '2551 - Outra UF - Compra de bem para o ativo imobilizado'
        '2556 - Outra UF - Compra de material para uso ou consumo'
        
          '2916 - Outra UF - Retorno de mercadoria ou bem remetido para con' +
          'serto ou reparo'
        
          '2922 - Outra UF - Lan'#231'amento efetuado a t'#237'tulo de simples fatura' +
          'mento decorrente da compra'
        '5101 - Venda de produ'#231#227'o do estabelecimento'
        '5102 - Venda de mercadoria adquirida ou recebida de terceiros'
        
          '5210 - Devolu'#231#227'o de compra para utiliza'#231#227'o na presta'#231#227'o de servi' +
          #231'o'
        
          '5403 - Venda de mercadoria adquirida ou recebida de terceiros em' +
          ' ST'
        
          '5405 - Venda de mercadoria adquirida ou recebida de terceiros em' +
          ' ST - Constribuinte Substituto'
        '5915 - Remessa de mercadoria ou bem para conserto ou reparo'
        
          '5929 - Lan'#231'amento efetuado em decorr'#234'ncia de emiss'#227'o de document' +
          'o ECF'
        '6101 - Outra UF - Venda de produ'#231#227'o do estabelecimento'
        
          '6102 - Outra UF - Venda de mercadoria adquirida ou recebida de t' +
          'erceiros'
        
          '6210 - Outra UF - Devolu'#231#227'o de compra para utiliza'#231#227'o na presta'#231 +
          #227'o de servi'#231'o'
        
          '6403 - Outra UF - Venda de mercadoria adquirida ou recebida de t' +
          'erceiros em ST'
        
          '6405 - Outra UF - Venda de mercadoria adquirida ou recebida de t' +
          'erceiros em ST - Constribuinte Substituto'
        
          '6915 - Outra UF - Remessa de mercadoria ou bem para conserto ou ' +
          'reparo'
        
          '6929 - Outra UF - Lan'#231'amento efetuado em decorr'#234'ncia de emiss'#227'o ' +
          'de documento ECF')
      TabOrder = 0
      Visible = False
    end
    object CheckListBox1: TCheckListBox
      Left = 2
      Top = 22
      Width = 734
      Height = 208
      ItemHeight = 13
      TabOrder = 1
    end
  end
  object DBGrid1: TDBGrid
    Left = 187
    Top = 350
    Width = 549
    Height = 158
    DataSource = dm_DADOS.zdts_ESPELHO
    TabOrder = 3
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = []
  end
  object BitBtn2: TBitBtn
    Left = 8
    Top = 67
    Width = 75
    Height = 25
    Caption = 'BitBtn2'
    TabOrder = 4
    Visible = False
    OnClick = BitBtn2Click
  end
  object BitBtn3: TBitBtn
    Left = 89
    Top = 98
    Width = 75
    Height = 25
    Caption = 'BitBtn3'
    TabOrder = 5
    Visible = False
    OnClick = BitBtn3Click
  end
  object BitBtn4: TBitBtn
    Left = 89
    Top = 67
    Width = 75
    Height = 25
    Caption = 'BitBtn4'
    TabOrder = 6
    Visible = False
  end
  object btn_MARCAR: TBitBtn
    Left = 265
    Top = 8
    Width = 235
    Height = 20
    Caption = 'Marcar Todos'
    TabOrder = 7
    OnClick = btn_MARCARClick
  end
  object btn_DESMARCAR: TBitBtn
    Left = 505
    Top = 8
    Width = 235
    Height = 20
    Caption = 'Desmarcar Todos'
    TabOrder = 8
    OnClick = btn_DESMARCARClick
  end
  object BitBtn5: TBitBtn
    Left = 8
    Top = 511
    Width = 728
    Height = 25
    Caption = 'CONSULTA MOVIMENTO DO PRODUTO = INVENTARIO | ENTRADA | SAIDA'
    TabOrder = 9
    OnClick = BitBtn5Click
  end
  object BitBtn6: TBitBtn
    Left = 8
    Top = 459
    Width = 173
    Height = 49
    Caption = 'Gerar Visualiza'#231#227'o por Produto'
    TabOrder = 10
    OnClick = BitBtn6Click
  end
  object edt_cod: TEdit
    Left = 100
    Top = 31
    Width = 70
    Height = 21
    TabOrder = 11
  end
  object BitBtn1: TBitBtn
    Left = 8
    Top = 357
    Width = 173
    Height = 103
    Caption = 'Gerar Visualiza'#231#227'o'
    Glyph.Data = {
      
      FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF}
    Layout = blGlyphTop
    TabOrder = 12
    OnClick = BitBtn1Click
  end
  object BitBtn7: TBitBtn
    Left = 265
    Top = 28
    Width = 235
    Height = 20
    Caption = 'Agrupa Compras'
    TabOrder = 13
    OnClick = BitBtn7Click
  end
  object BitBtn8: TBitBtn
    Left = 505
    Top = 28
    Width = 235
    Height = 20
    Caption = 'Agrupa Vendas'
    TabOrder = 14
    OnClick = BitBtn8Click
  end
  object BitBtn9: TBitBtn
    Left = 8
    Top = 536
    Width = 728
    Height = 25
    Caption = 
      'CONSULTA PRODUTOS COM CUSTO DIVERGENTE - (Custo Informado - Cust' +
      'o PEPS - Custo M'#233'dio'
    TabOrder = 15
    OnClick = BitBtn9Click
  end
  object BitBtn10: TBitBtn
    Left = 265
    Top = 88
    Width = 475
    Height = 30
    Caption = 'Refazer'
    TabOrder = 16
    OnClick = BitBtn10Click
  end
  object btn_indcom_compra: TBitBtn
    Left = 265
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Ind'#250'stria e Com'#233'rcio'
    Caption = 'Ind e Com.'
    TabOrder = 17
    OnClick = btn_indcom_compraClick
  end
  object btn_devolucao_compra: TBitBtn
    Left = 312
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Devolu'#231#245'es de Compras'
    Caption = 'btn_devolucao_compra'
    TabOrder = 18
    OnClick = btn_devolucao_compraClick
  end
  object btn_trans_compra: TBitBtn
    Left = 359
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Transfer'#234'ncias de Compras'
    Caption = 'btn_trans_compra'
    TabOrder = 19
    OnClick = btn_trans_compraClick
  end
  object btn_ativo_compra: TBitBtn
    Left = 406
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Ativo Imobilizado'
    Caption = 'btn_ativo_compra'
    TabOrder = 20
    OnClick = btn_ativo_compraClick
  end
  object btn_outros_compra: TBitBtn
    Left = 453
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Outras Entradas e Aquisi'#231#227'o de Servi'#231'os'
    Caption = 'btn_outros_compra'
    TabOrder = 21
    OnClick = btn_outros_compraClick
  end
  object btn_indcom_venda: TBitBtn
    Left = 505
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Sa'#237'das e Presta'#231#227'o de Servi'#231'os'
    Caption = 'btn_indcom_venda'
    TabOrder = 22
    OnClick = btn_indcom_vendaClick
  end
  object btn_devolucao_venda: TBitBtn
    Left = 552
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Devolu'#231#245'es de Sa'#237'da'
    Caption = 'btn_devolucao_venda'
    TabOrder = 23
    OnClick = btn_devolucao_vendaClick
  end
  object btn_trans_venda: TBitBtn
    Left = 599
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Transfer'#234'ncias de Vendas'
    Caption = 'btn_trans_venda'
    TabOrder = 24
    OnClick = btn_trans_vendaClick
  end
  object btn_ativo_venda: TBitBtn
    Left = 646
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Ativos Imobilizados'
    Caption = 'btn_ativo_venda'
    TabOrder = 25
    OnClick = btn_ativo_vendaClick
  end
  object btn_outros_venda: TBitBtn
    Left = 693
    Top = 48
    Width = 47
    Height = 40
    Hint = 'Outras Vendas e Presta'#231#227'o de Servi'#231'os'
    Caption = 'btn_outros_venda'
    TabOrder = 26
    OnClick = btn_outros_vendaClick
  end
end
