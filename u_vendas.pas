...
begin
  ...
  begin
    showmessage('Campos de preenchimento Obrigatório estão pendentes');
  end
  else
  begin
    ...
    if cb_tipomov.Text='ORÇAMENTO' then zqry_CADASTROTIPOVENDA.AsString:='O';
...
begin
  ...
  begin
    cb_tipomov.Items.Add('Orçamento');
  end;
  //
...
begin
  ...
  begin
    showmessage('Dados deste produto não foram localizados');
    cb_PRODUTO.SetFocus;
  end
  else
  begin
...
begin
  ...
  if cb_tipomov.Text='Orçamento' then
  begin
    edt_serie.Enabled:=false;
    edt_serie.Text:='OC';
  end;
...
begin
  ...
  begin
    edt_VLRUNITARIO.SetFocus;
    showmessage('Custo do produto está sendo comprometido nessa venda');
  end;
end;
...
begin
 ...
 // showmessage(inttostr(seqitemvenda));
  ...
  begin
    showmessage('Produtos não localizados');
  end;
